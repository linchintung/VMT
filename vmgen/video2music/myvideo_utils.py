...
class VideoProblem(problem.Problem):
  ...
  def max_frames_per_video(self, hparams):
    ...
    return hparams.video_num_input_frames
  ...
  def preprocess(self, dataset, mode, hparams, interleave=True):
    ...
    def features_from_batch(batched_prefeatures):
      ...
      """
      # tf.logging.fatal('in video_utils.py preprocess... #441')
      # import IPython
      # IPython.embed()
      features = {}
      for k, v in six.iteritems(batched_prefeatures):
        ...
        if k == "frame":  # We rename past frames to inputs and targets.
          ...
          # tf.logging.fatal('in video_utils.py preprocess... #451')
          # import IPython
          # IPython.embed()
          # nv = v[-1][:hparams.max_target_seq_length]
          nv = v[-1]
          features["targets"] = tf.pad(nv, [[0, hparams.max_target_seq_length - tf.shape(nv)[0]]], "CONSTANT")
        #   s1, s2 = split_on_batch(v)
        #   features["targets"] = s2
        else: #frame_number
          # s1, s2 = split_on_batch(v)
          features["input_%s" % k] = v
    ...
    def avoid_break_batching(dataset):
      ...
      # from video2music.logging import logger; logger().fatal('Pa!')
      # import IPython; IPython.embed()

      # ds = [dataset.skip(i) for i in range(num_frames)]

      # from video2music.logging import logger; logger().fatal('Pe!')
      # import IPython; IPython.embed()

      # dataset = tf.data.Dataset.zip(tuple(ds))

      # from video2music.logging import logger; logger().fatal('Pi!')
      # import IPython; IPython.embed()

      # dataset = dataset.map(check_integrity_and_batch)

      # from video2music.logging import logger; logger().fatal('Po!')
      # import IPython; IPython.embed()

      # dataset = dataset.filter(lambda _, not_broken: not_broken)

      # from video2music.logging import logger; logger().fatal('Pu!')
      # import IPython; IPython.embed()

      # dataset = dataset.map(lambda features, _: features)

      dataset = dataset.batch(batch_size=num_frames, drop_remainder=False)

      # from video2music.logging import logger; logger().fatal('Py!')
      # import IPython; IPython.embed()

      return dataset

    # from video2music.logging import logger; logger().fatal('Ta!')
    # import IPython; IPython.embed()

    preprocessed_dataset = dataset.map(_preprocess)

    # from video2music.logging import logger; logger().fatal('Te!')
    # import IPython; IPython.embed()
    ...
    # from video2music.logging import logger; logger().fatal('Ti!')
    # import IPython; IPython.embed()
    ...
    # from video2music.logging import logger; logger().fatal('To!')
    # import IPython; IPython.embed()
    ...
    # from video2music.logging import logger; logger().fatal('Tu!')
    # import IPython; IPython.embed()


    # tf.logging.fatal('in video_utils.py preprocess...')
    # import IPython
    # IPython.embed()
    ...
    # from video2music.logging import logger; logger().fatal('Ty!')
    # import IPython; IPython.embed()

    # tf.logging.fatal('in video_utils.py preprocess... #548')
    # import IPython
    # IPython.embed()
