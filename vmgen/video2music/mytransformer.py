...
def transformer_encode(encoder_function, inputs, target_space, hparams,
                       ...
                       **kwargs):
  """Encode mytransformer inputs.

  Args:
    encoder_function: the encoder function
    inputs: MyTransformer inputs [batch_size, input_length, 1, hidden_dim] which
...
def transformer_decode(decoder_function,
                       ...
                       losses=None,
                       **kwargs):
  """Decode MyTransformer outputs from encoder representation.
...
def savp_encoder(inputs, hparams, n_layers=3):
  ...
  """
  # latent_dims = hparams.z_dim
  latent_dims = hparams.hidden_size
  ...
  # batch_size, num_frames = shape_as_list[:2]
  batch_size, num_frames = tf.shape(inputs)[0], tf.shape(inputs)[1]
  ...
  # tf.logging.fatal('in savp encoder... #241')
  # import IPython
  # IPython.embed()
...
class Mytransformer(t2t_model.T2TModel):
  ...
  def __init__(self, *args, **kwargs):
    ...
    self._savp_encoder = savp_encoder

  def encode(self, inputs, target_space, hparams, features=None, losses=None):
    """Encode mytransformer inputs, see transformer_encode."""
  ...
  def decode(self,
             ...
             losses=None,
             **kwargs):
    """Decode myTransformer outputs, see transformer_decode."""
  ...
  def body(self, features):
    """MyTransformer main model_fn.

    Args:
      features: Map of features to the model. Should contain the following:
          "inputs": MyTransformer inputs. [batch_size, input_length, 1,
            hidden_dim].
    ...
    # tf.logging.fatal('in mytransformer body... #307')
    # import IPython
    # IPython.embed()

    pooled = self.video_encode(inputs, hparams=hparams)
    ...
    # new_input = []
    # for i in range(common_layers.shape_list(inputs)[1]):
    #   encoder_input = cia.prepare_encoder(inputs[:, i, :, :, :], hparams, "local_2d")
    ...
    #       name="encoder")
    #   new_input.append(encoder_output)
    # new_input = tf.stack(new_input, axis=1)
    
    # tf.logging.fatal('in mytransformer body... #333')
    # import IPython
    # IPython.embed()
    ...
    #   tf.logging.fatal('in mytransformer body... #247')
    #   IPython.embed()
    # else:
    #   encoder_output, encoder_decoder_attention_bias = (None, None)

    encoder_output, encoder_decoder_attention_bias = self.encode(
          pooled, target_space, hparams, features=features, losses=losses)
    ...
    # Not all subclasses of MyTransformer support keyword arguments related to
  ...
  def _greedy_infer(self, features, decode_length, use_tpu=False):
    ...
    if (self._target_modality_is_real or
      ...
      return super(Mytransformer, self)._greedy_infer(features, decode_length)
  ...
  def _fast_decode(self,
    ...
    if self.has_input:
      ...
      # from video2music.logging import logger; logger().fatal('ia!')
      ...
      # from video2music.logging import logger; logger().fatal('inputs!')
      # import IPython; IPython.embed()
    ...
    # from video2music.logging import logger; logger().fatal('decode_length!')
    # import IPython; IPython.embed()
    ...
    # from video2music.logging import logger; logger().fatal('ret!')
    # import IPython; IPython.embed()
...
def _init_transformer_cache(cache, hparams, batch_size, attention_init_length,
                            ...
                            scope_prefix):
  """Create the initial cache for MyTransformer fast decoding."""
...
@registry.register_model
class MyTransformerScorer(Mytransformer):
  """MyTransformer model, but only scores in PREDICT mode.

  Checkpoints between MyTransformer and MyTransformerScorer are interchangeable.
  """

  def __init__(self, *args, **kwargs):
    super(MyTransformerScorer, self).__init__(*args, **kwargs)
...
@registry.register_model
class MyTransformerRegressor(MyTransformerEncoder):
...
@registry.register_model
class MyTransformerMemory(Mytransformer):
  """MyTransformer language model with memory across chunks."""
  ...
  def has_input(self):
    if hasattr(self._hparams, "unconditional") and self._hparams.unconditional:
      return False
    return super(MyTransformerMemory, self).has_input
...
@registry.register_hparams
def mytransformer_base_v2():
  """Set of hyperparameters."""
  hparams = mytransformer_base_v1()
...
@registry.register_hparams
def mytransformer_base():
  """Base parameters for Transformer model."""
  hparams = mytransformer_base_v3()
  return hparams


@registry.register_hparams
def mytransformer_big():
  """HParams for transformer big model on WMT."""
  hparams = mytransformer_base()
...
@registry.register_hparams
def mytransformer_tall():
  """Hparams for on LM for pretraining/finetuning/mixing."""
  hparams = mytransformer_base()
  hparams.batch_size = 2048
...
@registry.register_hparams
def mytransformer_tall_finetune_tied():
  """Tied means fine-tune CNN/DM summarization as LM."""
  hparams = mytransformer_tall()
...
@registry.register_hparams
def mytransformer_tall_train_tied():
...
@registry.register_hparams
def mytransformer_tall_finetune_uniencdec():
...
@registry.register_hparams
def mytransformer_tall_train_uniencdec():
...
@registry.register_hparams
def mytransformer_tall_finetune_textclass():
...
@registry.register_hparams
def mytransformer_tall_pretrain_lm():
...
@registry.register_hparams
def mytransformer_big_single_gpu():
  """HParams for transformer big model for single GPU."""
  hparams = mytransformer_big()
  hparams.layer_prepostprocess_dropout = 0.1
...
@registry.register_hparams
def mytransformer_base_single_gpu():
...
@registry.register_hparams
def mytransformer_base_multistep8():
...
@registry.register_hparams
def mytransformer_parsing_base():
...
@registry.register_hparams
def mytransformer_parsing_ice():
  """HParams for parsing and tagging Icelandic text."""
  hparams = mytransformer_base_single_gpu()
  hparams.batch_size = 4096
  hparams.shared_embedding_and_softmax_weights = False
  return hparams
...
@registry.register_hparams
def mytransformer_big_enfr():
  hparams = mytransformer_big_dr1()
  hparams.shared_embedding_and_softmax_weights = False
...
@registry.register_hparams
def mytransformer_parameter_attention_a():
  hparams = mytransformer_base()
...
@registry.register_hparams
def mytransformer_parameter_attention_b():
  hparams = mytransformer_base()
  hparams.ffn_layer = "parameter_attention"
  hparams.filter_size = 512
...
@registry.register_hparams
def mytransformer_prepend():
  return mytransformer_prepend_v2()


@registry.register_ranged_hparams
def mytransformer_base_range(rhp):
...
@registry.register_hparams
def mytransformer_relative_tiny():
  hparams = mytransformer_relative()
  hparams.num_hidden_layers = 2
  hparams.hidden_size = 128
  hparams.filter_size = 512
  hparams.num_heads = 4
...
@registry.register_hparams
def mytransformer_relative_big():
  hparams = mytransformer_big()
  hparams.pos = None
  hparams.self_attention_type = "dot_product_relative"
  hparams.max_relative_position = 20
...
@registry.register_hparams
def mytransformer_timeseries():
  hparams = mytransformer_small()
...
@registry.register_hparams
def mytransformer_fairseq_fp16_activation_big():
...
@registry.register_hparams
def mytransformer_clean_big():
  hparams = mytransformer_clean()
  hparams.hidden_size = 1024
  hparams.filter_size = 4096
  return hparams
...
@registry.register_hparams
def mytransformer_librispeech_v1():
...
@registry.register_hparams
def mytransformer_librispeech_v2():
...
@registry.register_hparams
def mytransformer_librispeech():
  """HParams for training ASR model on Librispeech."""
  return mytransformer_librispeech_v2()


@registry.register_hparams
def mytransformer_common_voice():
  """HParams for training ASR model on Mozilla Common Voice."""
  return mytransformer_librispeech()


@registry.register_hparams
def mytransformer_supervised_attention():
...
@registry.register_hparams
def mytransformer_cifar10_memory_v0():
  """HParams for training image_cifar10_plain_gen_flat_rev with memory."""
  hparams = mytransformer_wikitext103_l4k_memory_v0()
...
@registry.register_hparams
def mytransformer_imagenet64_memory_v0():
  """HParams for training image_imagenet64_gen_flat_rev with memory."""
  hparams = mytransformer_cifar10_memory_v0()
